  ...
  return (
    <section id="services" className="py-24 max-w-7xl mx-auto px-6">
      <div className="mb-16">
        ...
        <h3 className="font-light text-4xl text-white">The 3-Tier Model</h3>
        <p className="text-slate-400 mt-4 max-w-2xl font-light">Scalable governance structures designed for different maturity stages of enterprise risk.</p>
      </div>

      <div className="grid md:grid-cols-3 gap-6">
        {tiers.map((tier, idx) => (
          <div key={idx} className={`border p-8 flex flex-col h-full relative ${tier.recommended ? 'border-indigo-500/20 bg-[#111] shadow-2xl shadow-indigo-500/5' : 'border-white/5 bg-[#111]/30'}`}>
            {tier.recommended && (
            ...
            <h4 className="font-medium text-2xl text-white mb-6">{tier.title}</h4>
            <ul className="space-y-4 mb-8 flex-1">
              {tier.features.map((feature, fIdx) => (
                <li key={fIdx} className="flex items-start gap-3">
                  <CheckCircle2 className={`w-4 h-4 shrink-0 mt-0.5 ${tier.recommended ? 'text-indigo-400' : 'text-slate-500'}`} />
                  <span className={`text-sm font-light leading-relaxed ${tier.recommended ? 'text-slate-300' : 'text-slate-400'}`}>{feature}</span>
                </li>
              ))}
            </ul>
